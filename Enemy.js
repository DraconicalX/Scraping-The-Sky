...
class Enemy{
  ...
  move(playerX,playerY){
    ...
    if(playerX+width/2 -50-this.x!=0){
      this.x+=((playerX+width/2 -50-this.x)/abs(playerX+width/2 -50-this.x))*enemyStats.SPD
    }
    if(playerY+height/2 -50-this.y!=0){
      this.y+=((playerY+height/2 -50-this.y)/abs(playerY+height/2 -50-this.y))*enemyStats.SPD
    }
  }
